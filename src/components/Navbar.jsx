...
const Navbar = () => {
  ...
  // Handle scroll to hide on scroll down and reveal on scroll up
  useEffect(() => {
    const handleScroll = () => {
  ...
  const hireMeMailto = `mailto:${personalInfo.emails.primary}?subject=Hiring Inquiry – Portfolio&body=Hello Mohammed Ahsan,%0D%0A%0D%0AI came across your portfolio and would like to discuss an opportunity with you.%0D%0A%0D%0ALooking forward to hearing from you.%0D%0ABest Regards,`;
    ...
    <nav 
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 transform ${
        ...
        isOpen 
          ...
          : isScrolled 
            ? 'bg-black/60 backdrop-blur-md py-4 shadow-lg border-b border-white/10' 
            : 'bg-transparent py-6'
      }`}
    >
